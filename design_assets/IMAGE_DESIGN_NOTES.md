...
## 6.6 版本五元组与可复现流水线（图 6.6-1）

- 文件：`chapters/06-software-tools-simulation/assets/images/06-g1-version-quintuple-reproducibility.png`（原第 7 章 7.1 节的配图，随小节迁入第六章，文件名同步改为 06- 前缀）
- 内容：决定系统行为的五个版本化元素（代码、模型、参数、固件、数据）及各自版本载体，以及“环境锁定—随机种子—日志数据—结果归档”的可复现流水线
- 来源与处理：本书自制示意图，未引用外部素材
- 用途：6.6「版本管理：不止 Git」之后