...
1.1 已经定义过闭环：智能体持续感知、决策、行动，再用行动结果修正下一步。这里不再重复概念，而是看这条环在人形机器人整机架构里具体落在哪些层：

```text
传感器采集（电气与嵌入式层）
→ 状态估计（小脑层）
→ 感知、规划或 VLA/WAM 决策（大脑层）
→ 运动规划与全身控制（大脑层 → 小脑层）
→ 关节伺服与电机驱动（电气与嵌入式层）
→ 本体运动并产生新观测（机械与本体层）
```

这就是反馈控制（Feedback Control）思想在整机层面的落地。[4] 大脑层通常以较低频率更新任务和动作意图，小脑层以更高频率维护平衡和跟踪目标，电气与嵌入式层再把这些目标按固定周期交给执行器。缺少任何一环，机器人就无法形成稳定的自主闭环。
