...
> **在整机里的位置**：这一节属于“机械执行层”；上游是驱动与嵌入式层的力矩，下游是传感器与状态估计。总图见 [1.4 节](../01-system-overview/04-from-task-to-motor.mdx)。
...
> **一句话听懂**：执行器是“真正出力的部件”——电机加减速器组成关节模组，规格书里的力矩与转速说的就是它。
...
> **一句话听懂**：传感器分两类——量自己的（编码器量关节角、IMU 量姿态）和量世界的（相机、激光雷达、足底力）；上层算法拿到的“状态”和“环境”都从它们来。
...
> **一句话听懂**：末端执行器是运动链最末端“跟世界打交道”的部件——足部跟地面打交道，手掌与手指跟物体打交道；它和执行器（出力的部件）只差一个字，别混。
...
你能旋转的这台 G1，在软件里首先是一份**机器人描述文件**（Robot Description File）：一份文本写清楚机器人由哪些刚体（Link）组成、它们怎么连接（Joint）、每个关节能怎样运动。同一份文件，机械同学拿它核对结构与安装，算法同学拿它算运动学与动力学，仿真器拿它生成接触与执行器——它是机械的交付物，也是算法的输入。最常用的两种格式是 URDF（Unified Robot Description Format，偏结构描述与 ROS 生态）和 MJCF（MuJoCo XML Format，把场景、执行器和传感器一起写入），字段细节在 [2.5 节](../02-mechanics/05-robot-description-files.mdx)展开。点一下模型上的任意部件，右侧会跳到它在 `g1_29dof.urdf` 里定义的那一行——这就是这份契约的样子。
...
| 变体 | 腰部 | 每条手臂 | 手 | 总 DoF |
| --- | --- | --- | --- | --- |
| `g1_23dof` | 1（只保留 yaw） | 5（腕部少 pitch 与 yaw 两个自由度，保留 roll） | 无 | 23 |
| `g1_29dof` | 3 | 7 | 无 | 29 |
| `g1_29dof_with_hand` | 3 | 7 | 每手 7（三指） | 29 + 14 |

差在哪：23 DoF 比 29 DoF 少的是腰部的 roll/pitch 与每条手臂腕部的 2 个自由度；SDK 的索引注释会把 `LeftWristPitch`/`LeftWristYaw`（右侧 27/28）标为 `NOTE INVALID for g1 23dof`[8]。本手册统一用 `g1_29dof`；需要锁腰对比时用 `g1_29dof_lock_waist`，需要操作时用 `g1_29dof_with_hand`。
还要记住一条边界：**仿真模型里的刚体和关节，不一定完整反映真实硬件中的柔性、摩擦和热限制**。[5]
...
[8] Unitree Robotics. *unitree_sdk2：G1 低层示例与关节索引定义*. 本文使用提交 `9754cd153af3da471b0fe5f3aa535e426fb11db3`，用于核对关节索引别名与 23 DoF 上无效的编号。<https://github.com/unitreerobotics/unitree_sdk2/tree/9754cd153af3da471b0fe5f3aa535e426fb11db3/example/g1/low_level>
