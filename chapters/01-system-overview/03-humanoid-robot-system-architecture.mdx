...
这条链路上有两个名字值得先记住：`LowCmd` 和 `LowState` 是 G1 低层通信接口的核心消息——`LowCmd` 逐关节携带目标位置、速度、前馈力矩和 `kp`/`kd` 增益，`LowState` 回报电机状态和 IMU 数据。[9][10] 也就是说，上层算法和真实电机之间隔着这两个消息。下面是从官方 SDK 和官方低层示例中截取的真实定义，高亮行就是上面提到的字段；`kp`/`kd`/`tau` 的控制含义在 4.5 节展开。
