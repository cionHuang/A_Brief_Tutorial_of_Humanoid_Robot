...
强化学习的最小词汇：**状态** $s$、**动作** $a$、**奖励** $r$、**策略** $\pi(a\mid s)$、**价值函数** $V^\pi(s)$[1]；目标通常是最大化折扣回报 $\mathbb{E}\left[\sum_t \gamma^t r_t\right]$。按数据来源分**离线**（用固定数据集）与**在线**（边跑边学）；按更新对象分 **on-policy**（只用自己的数据）与 **off-policy**（可用历史数据）。细节见 5.5 节。
