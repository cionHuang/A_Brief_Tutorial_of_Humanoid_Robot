...
先交代两个后面反复出现的缩写：**VLA**（Vision-Language-Action，视觉-语言-动作模型，见 5.6 节）与 **WAM**（World Action Model，世界动作模型，见 5.7 节）。也先划清章节边界：第五章是**大脑**层（感知、规划、操作、学习、VLA/WAM），回答“去做什么”；**小脑**层（状态估计、平衡与全身控制）是第四章，回答“这一毫秒怎么动”。这一章从一套词汇清单开始（5.1），然后依次是感知（5.2）、规划（5.3）、操作（5.4）、学习（5.5），最后是 VLA（5.6）与 WAM（5.7）。
...
- 动作的表示不止一种：离散成若干档的**动作 token**（像“生成词”一样逐个生成）[5]、直接回归连续向量，以及扩散/流匹配生成一整段动作。前两者简单，但分档会损失精度、回归会把多种可行解平均掉；生成式能表达多峰，代价是要迭代多步（见后面的“生成：扩散与流匹配”与 5.6 节）。
...
为什么 VLA/WAM 选它：图像、语言、机器人状态和动作可以统一成一条序列处理，容量大、易扩展。代价是**计算量和显存随序列长度增长**，而机器人对延迟敏感——这正是“模型很大、但推理频率仍受限”的根源（见 5.6 节）。
...
两者都能一次生成一整段**动作块（Action Chunk）**，用“一次推理出一串动作”来掩盖推理延迟；代价是采样需要迭代多步，步数越多、延迟越大（见 5.6 节）。
...
强化学习的最小词汇：**状态** $s$、**动作** $a$、**奖励** $r$、**策略（Policy）** $\pi(a\mid s)$、**价值函数（Value Function）** $V^\pi(s)$[1]；目标通常是最大化折扣回报 $\mathbb{E}\left[\sum_t \gamma^t r_t\right]$。按数据来源分**离线**（用固定数据集）与**在线**（边跑边学）；按更新对象分 **on-policy**（只用自己的数据）与 **off-policy**（可用历史数据）。细节见 5.5 节。

## 具身特有的四个坑

1. **样本效率**：真机上试错一次代价很高，所以大量训练放在仿真里；
2. **Sim2Real 差距**：动力学、摩擦、传感器噪声和执行延迟在仿真与真机之间总有差别，常用域随机化和系统辨识来缩小（见 5.5 节）；
...
| --- | --- | --- | --- | --- |
| 任务与行为（见 5.6 的长任务分解） | 任务分解、技能调度 | 语言指令 + 世界状态 → 子任务、技能调用 | 0.1–10 Hz | 只组织“先做什么”，不做运动求解 |
