...
本节是第五章的**词汇地图**：不推导算法、不涉及具体机型参数，只把后面几节会反复出现的现代 AI 与机器人学习词汇讲成人话。读完这一节再读 5.2–5.8，遇到这些词时能立刻知道它在说哪一层的事。
...
4. **实时性与安全**：模型推理低频且可能出错，输出的每个动作都必须经过限幅、碰撞检查与可行性检查，再由确定性的控制层执行（见 5.8 节）。
...
| 到 G1 接口（5.8） | 解码、IK、WBC、安全过滤 | 动作意图 → 关节命令 | 与底层同频 | 安全过滤一层都不能省 |
