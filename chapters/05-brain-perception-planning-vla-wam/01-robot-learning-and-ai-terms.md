...
- 动作的表示不止一种：离散成若干档的**动作 token**（像“生成词”一样逐个生成）、直接回归连续向量，以及扩散/流匹配生成一整段动作。前两者简单，但分档会损失精度、回归会把多种可行解平均掉；生成式能表达多峰，代价是要迭代多步（见前面的“生成：扩散与流匹配”与 5.6 节）。
...
强化学习的最小词汇：**状态** $s$、**动作** $a$、**奖励** $r$、**策略** $\pi(a\mid s)$、**价值函数** $V^\pi(s)$[5]；目标通常是最大化折扣回报 $\mathbb{E}\left[\sum_t \gamma^t r_t\right]$。按数据来源分**离线**（用固定数据集）与**在线**（边跑边学）；按更新对象分 **on-policy**（只用自己的数据）与 **off-policy**（可用历史数据）。细节见 5.5 节。
