...
先交代两个后面反复出现的缩写：**VLA**（Vision-Language-Action，视觉-语言-动作模型，见 5.6 节）与 **WAM**（World Action Model，世界动作模型，见 5.7 节）。也先划清章节边界：第五章是**大脑**层（感知、规划、操作、学习、VLA/WAM），回答“去做什么”；**小脑**层（状态估计、平衡与全身控制）是第四章，回答“这一毫秒怎么动”。这一章从一套词汇地图开始（5.1），然后依次是感知（5.2）、规划（5.3）、操作（5.4）、学习（5.5），最后是 VLA（5.6）与 WAM（5.7）。
