...
- **动力学差异**：接触模型、减速器摩擦与回差、执行器响应。仿真器之间也有差异，所以先在另一个仿真器里验证（[Sim2Sim](../06-software-tools-simulation/06-sim-to-real.md)，详见 6.6 节）——例如在 [Isaac Gym（6.3 节的 Isaac Lab 是其继任）](../06-software-tools-simulation/03-isaac-sim-isaac-lab.md) 里训练、到 [MuJoCo（6.2 节）](../06-software-tools-simulation/02-mujoco.md) 里部署——能用很低成本暴露"策略过拟合了训练仿真器"的问题；[5][7]
...
以 [`unitree_rl_gym`](https://github.com/unitreerobotics/unitree_rl_gym) 的 G1 行走为例，训练到部署分四步，每一步在官方仓库里都有对应目录（三个仓库都是 BSD-3-Clause）：[7]

| 阶段 | 做什么 | 对应仓库与目录 |
| --- | --- | --- |
| 并行训练 | Isaac Gym 里成千上万个环境同时跑 PPO + 域随机化 | [`unitree_rl_gym/legged_gym/`](https://github.com/unitreerobotics/unitree_rl_gym/tree/main/legged_gym)：训练入口 `scripts/train.py`，G1 的观测、奖励与课程在 `envs/g1/` |
| Sim2Sim | 把同一策略换到 MuJoCo 跑，确认它不依赖训练仿真器的特性 | 入口 [`deploy/deploy_mujoco/deploy_mujoco.py`](https://github.com/unitreerobotics/unitree_rl_gym/tree/main/deploy/deploy_mujoco)，或官方 [`unitree_mujoco`](https://github.com/unitreerobotics/unitree_mujoco)（`simulate/` 是仿真器，`unitree_robots/` 放各机型的 MJCF） |
| 部署 | 策略网络以 50 Hz 级频率输出各关节目标位置 | [`deploy/deploy_real/`](https://github.com/unitreerobotics/unitree_rl_gym/tree/main/deploy/deploy_real)：中文说明在 `README.zh.md`，C++ 版在 `cpp_g1/`；预训练策略在 [`deploy/pre_train/g1/`](https://github.com/unitreerobotics/unitree_rl_gym/tree/main/deploy/pre_train/g1) |
| 低层执行 | 按固定的 kp/kd 增益做 PD 跟踪 | [`unitree_sdk2`](https://github.com/unitreerobotics/unitree_sdk2) 的 `rt/lowcmd` 与 `MotorCmd_`（4.5 节） |
