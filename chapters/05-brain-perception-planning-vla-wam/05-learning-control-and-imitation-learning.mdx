...
注意这条链路里学习策略的边界：策略只决定“关节目标往哪走”，力矩级的稳定性、限幅和异常保护仍由低层和 [3.6 节](../03-electrical-embedded/06-electrical-safety-and-protection.mdx)的安全链路承担。策略输出必须做限幅和变化率限制，未经 Sim2Sim 验证的策略不上实机，实机首跑必须有吊架或人工保护。
