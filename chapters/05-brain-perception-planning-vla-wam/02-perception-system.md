...
感知系统（Perception System）负责把相机、深度传感器等原始数据转换成关于环境的结构化判断：那里有什么、在哪里、在什么坐标系、置信度多高。[1][2] 它和 4.4 节的状态估计分工明确：状态估计回答“我自己在哪、怎么动”，感知回答“周围有什么、它们在哪”。本章后续的操作、VLA/WAM 小节都要消费感知的输出。传感器本身的原理、安装与时间同步在 3.3 节，本节只讲感知怎么把它们的输出变成可用的结构化判断。

> **一句话听懂**：机器人同时维护两张“地图”：一张是自己的状态（4.4 节），一张是周围的环境（本节）；两张图必须共用同一个时间轴和坐标系，否则“看到杯子”和“手在哪”就对不上。
...
标定（Calibration）分两层，内参和外参的一般定义见 [3.3 节](../03-electrical-embedded/03-sensors-and-data-acquisition.md)，用到相机上就是：内参（Intrinsics）是相机的焦距 `fx, fy`、主点 `cx, cy` 和畸变系数，决定“像素怎么还原成一条射线”；外参（Extrinsics）是相机相对机器人某个 link 的安装位姿，决定“相机坐标系里的点落在身体的哪里”。[1]
...
> **一句话听懂**：里程计负责“连续但会漂”，SLAM 负责“全局一致但会跳”；两者都不能替代 4.4 节的本体状态估计，融合时时间戳、坐标系和不确定度都要显式对齐。
