...
操作任务的数据通常来自遥操作（Teleoperation）：人通过 VR 手柄、主从臂或外骨骼驱动机器人完成任务，同时记录图像、机器人状态和动作。LeRobot 等开源框架把数据集格式、采集和训练流水线标准化，是本书采用的参考实现之一。[6] 数据格式里动作的维度定义必须与 5.4 节的动作空间一致——换动作空间等于换数据集。
