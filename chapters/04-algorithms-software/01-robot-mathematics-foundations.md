...
前文[四连杆机构与死点](../02-mechanics/01-mechanisms-and-dof.md#四连杆机构与死点)讨论的是机构约束下的速度映射：输入关节速度经过机构雅可比 `J_mech` 才能变成输出端速度。接近共线构型时，传动角变差，`J_mech` 可能接近奇异，导致输出几乎不动、需要很大的输入速度，或越过死点后运动方向改变。万向节锁也可以写成一个局部映射：欧拉角速度经过 `E` 才变成真实角速度；但这里退化的是姿态参数化，不是机器人刚体或关节的机械传动能力。
