...
双足行走的底层几何参数是足步（Footstep）：步长决定前进速度的上限，步宽影响侧向稳定裕量，步高决定能否越过障碍和离地间隙。足步序列不是孤立的点列，它带时序：每一步的开始时刻、单支撑时长、双支撑时长共同构成步态时序（Gait Timing）。

4.6 节的捕获点给出"为了站稳下一步该踩哪"，足步规划在此之上加入地形、障碍和任务目标：能踩的位置与想踩的位置冲突时，用躯干补偿、调整迈步时长或降低步速来换取可行性。步频也不是越快越稳：单支撑时间缩短后，接触判断（4.4 节）和摆动腿轨迹的可用时间都同步缩短。
...
```text
minimize  跟踪误差 + 平滑代价 + 能耗/力矩代价
subject to  运动学/动力学约束、限位、碰撞、接触时序
```
...
![G1 运动规划的分层与时间轴](assets/images/08-g1-motion-planning-layers-timeline.png)

图：以 G1 `g1_29dof` 无手、腰部可动模型为例，对比同一起止点下笛卡尔直线路径与关节空间路径，展示任务层、规划层、MPC/WBC 层到关节控制层的频率分层、轨迹刷新的衔接点，以及梯形速度剖面与双支撑/单支撑步态时序。图中频率量级为典型示意；各层周期的具体数值以实际系统实现为准。
