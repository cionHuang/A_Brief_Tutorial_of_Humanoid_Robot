...
谐波减速器（Harmonic Drive / Strain-wave Gear）体积紧凑、减速比高、回差可以做得较小，目前常用于手腕这类对定位和体积敏感的关节。其柔性齿圈和波发生器会带来弹性与扭转刚度问题。
...
行星减速器（Planetary Gearbox）由太阳轮（Sun Gear）、行星轮（Planet Gear）和齿圈（Ring Gear）组成，承载能力和效率较好，在较高功率密度场景里用得较多。回差（齿隙）、轴承载荷和级间误差会影响输出精度。
...
摆线减速器（Cycloidal Reducer）利用偏心运动（Eccentric Motion）和滚柱（Roller）啮合承载冲击，抗冲击能力通常较强。但体积、噪声、摩擦和加工精度需要综合权衡。
...
> **第一次阅读如果只是建立系统概念，到这里即可。** 你已经拿到关节模组主线：电怎么变成关节端的力、一路在哪里损失。下面的热、寿命与保护，属于维护与选型的深入内容。

## 热、寿命与保护

关节的发热来源与降额行为见 [3.2 节](../03-electrical-embedded/02-motors-drivers-servo-control.mdx)：电机铜损、减速器与轴承摩擦都会转成热，温度过高时驱动器可能触发降额（Derating，也称热降额）或保护停机。
...
<SectionQA id="2.3" mode="answer" />

> **到这里，你应该已经能听懂**：“电机额定力矩不等于关节端可用力矩，中间还隔着减速器效率、摩擦、回差和发热”——以及“传动比”“峰值力矩”“持续力矩”“力矩—速度包络”“扭转刚度”“热降额”分别在说什么，选型时该同时核对哪几条边界。
