...
从机构角度看，死点与传动角（Transmission Angle）变差有关：有效传力方向趋于共线，驱动力矩难以转化为期望的输出运动。从算法角度看，同一个构型会使约束方程的雅可比矩阵（Jacobian）接近奇异（Singular），速度映射可能出现极大的关节速度或很小的末端速度，逆运动学（Inverse Kinematics, IK）和轨迹优化因此变得病态。[1][2][6]

下面的演示把这两种语言放在一起：左边是机械视角的四连杆，右边是同一条运动的输入输出曲线。机构默认恒速旋转，你可以拖动曲柄接管、停在任意构型上细看，松手后恢复恒速——注意接近死点时，输出摇杆的停顿和曲线切线的变平是同一时刻发生的：
...
这里的“雅可比接近奇异”与[第 4.1 节的欧拉角万向节锁](../04-cerebellum-realtime-control/01-robot-mathematics-foundations.md#与四连杆死点的联系)有一个重要联系：两者都表现为局部变量到物理量的映射秩下降或条件数恶化，但四连杆死点是实际机构的传动退化，万向节锁只是姿态参数化的表示退化。前者会改变输出运动和负载能力，后者不会让机器人刚体凭空少一个自由度。
