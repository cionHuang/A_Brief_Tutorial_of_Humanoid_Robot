...
> **一句话听懂**：URDF 把机器人拆成一堆 `link`，再用 `joint` 从根往末端串成一棵没有闭环的运动树。正因为是树、每个 link 只能有一个父节点，四连杆这类带闭环的机构就没法直接写进去，得靠插件、约束或转换后的近似模型。
...
> **一句话听懂**：`visual` 管好看、`collision` 管好撞、`inertial` 管重力算得对，同一套 link 里三套信息各自独立、可以完全不一样。所以显示网格精细不代表碰撞也精细，文件里写了 `inertial` 也说明不了材料强度、齿轮回差、结构柔性或电池热模型。
...
> **一句话听懂**：MJCF 用嵌套 `body` 搭好刚体骨架，再把关节、几何、执行器、传感器、接触和约束一起写进同一份 XML。也就是说一份 MJCF 自带仿真所需的世界和驱动接口，换格式时不能只看关节对不对，这些配置也得逐项翻译。
...
> **一句话听懂**：浮动基座就是机器人整体没被固定、额外带来的 6 个自由度。它不受关节限位，MJCF 里用 free 类型的 joint 表示；所以 `qpos`/`qvel` 的长度不等于电机数量，浮动基座、可动关节和传感器状态要分开数。
...
> **一句话听懂**：URDF 管“机器人长什么样、怎么动”，面向机器人结构本身；MJCF 在此基础上把“在什么世界里、怎么驱动、怎么感知”也一起写了。所以两者互相转换时，轴线、默认位姿、惯性、碰撞、限位和执行器都要逐项复核，不能只确认文件能被解析。
