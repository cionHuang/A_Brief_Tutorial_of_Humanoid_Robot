...
> **一句话听懂**：URDF 把机器人拆成一堆 `link`，再用 `joint` 从根往末端串成一棵没有闭环的运动树。
...
> **一句话听懂**：`visual` 管好看、`collision` 管好撞、`inertial` 管重力算得对，三套信息各自独立，可以完全不一样。
...
> **一句话听懂**：MJCF 用嵌套 `body` 搭好刚体骨架，再把关节、几何、执行器、传感器、接触和约束一起写进同一份 XML。
...
> **一句话听懂**：浮动基座就是机器人整体没被固定的 6 个自由度；数 `qpos`/`qvel` 时，要把它、电机和传感器分开算。
