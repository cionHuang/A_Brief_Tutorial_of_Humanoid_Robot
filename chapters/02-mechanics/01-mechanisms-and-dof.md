...
算法同学给 G1 下达“把左脚尖抬高 5 厘米”的目标，逆运动学（Inverse Kinematics, IK）却提示“目标不可达”；检查后发现，膝盖和踝关节都没有超出限位。机械同学重新标定零位后，脚尖确实到达了目标，却发现膝盖正负方向和仿真相反，接近目标时还会抖动。大家需要一起确认：模型中的连杆长度、关节轴线、自由度、零位和限位，是否与真实机构一致？

这是一条典型的机构调试线索：关节能够转动，不代表末端一定可达；即使目标可达，坐标方向、零位偏置或结构振动不一致，也会让同一组指令在仿真和实机上表现不同。
...
同学常问：“G1 是 29 DoF，为什么 URDF 里还能看到更多 joint？”这里的 29 DoF 统计独立运动变量；URDF 中的关节标签还可能包括固定关节、传感器安装关节等对象。

自由度（Degree of Freedom，DoF）表示描述一个机构构型所需的最少独立变量数量。一个只绕固定轴旋转的关节通常贡献 1 个 DoF；固定关节贡献 0 个 DoF；浮动基座（Floating Base）通常需要 6 个广义坐标描述整体位姿。[1][2]

G1 的 29 DoF 可以按功能区域粗略分为：双腿 12 DoF、腰部 3 DoF、双臂 14 DoF。这个统计对应无灵巧手、腰部未锁定的 29 DoF 版本；带手模型会增加手指关节，锁腰模型则会减少可动腰部自由度。确认 DoF 数量时，应查看具体模型文件和版本。[4]
...
给 G1 左腿一组关节角 `q_leg` 后，正运动学（Forward Kinematics, FK）根据髋、膝、踝的连杆变换计算脚端位姿；反过来，逆运动学（Inverse Kinematics, IK）根据期望脚端位姿求一组可能的关节角。多个 IK 解、关节限位和自碰撞检查，决定规划器最终是否接受这组解。[1][2]
