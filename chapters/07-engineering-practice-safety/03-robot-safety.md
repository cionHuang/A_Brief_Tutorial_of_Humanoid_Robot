...
软件层是日常事故最高发的一层，清单与 3.6 节一致：输出限幅、异常值与 `NaN` 校验、看门狗与超时、进程监控、故障状态机（初始化、运行、降级、故障、恢复）。开头的 `NaN` 事故说明一个原则：异常值检查必须放在消息链的每一跳，指望上游不产生 `NaN` 属于把安全建立在假设上。学习类组件（5.4–5.6 节）的输出更要全部经过这层过滤——会幻觉的组件不能绕过确定性的安全层。
