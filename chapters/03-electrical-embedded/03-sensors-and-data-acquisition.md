...
```text
q = qraw × scale + qoffset
```

`qraw` 是原始计数，`scale` 是计数到弧度的比例，`qoffset` 是机械装配零位与模型零位之间的偏置。速度 `dq` 常由相邻角度差分得到，但差分会放大量化噪声，实际系统需要滤波、观测器或驱动器内部速度估计。
...
`R` 是姿态旋转矩阵，`ω` 是传感器坐标系中的角速度，`Δt` 是采样间隔，`Exp` 表示旋转指数映射，`[·]×` 是叉乘矩阵。长时间积分会受到陀螺仪零偏影响，因此通常要结合重力方向、足底接触或其他外部观测进行校正。
