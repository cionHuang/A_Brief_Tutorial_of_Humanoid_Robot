...
（这一段里的三个名词——四元数分量顺序、浮动基座的速度维度、表达坐标系——本节正文都会逐个解释。第一遍读只要记住一件事：**“库和仿真器算得不一样”多半是约定不一致，不是算法不一致。**）
...
> **在整机里的位置**：这一节属于“横向基础设施：仿真与工具”；上游是模型与状态，下游是控制器与规划器。总图见 [1.4 节](../01-system-overview/04-from-task-to-motor.mdx)。
