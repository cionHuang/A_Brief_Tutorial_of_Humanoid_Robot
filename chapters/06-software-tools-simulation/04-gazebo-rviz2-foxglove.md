...
这三个现象在本节各有回收点：“RViz 里对、Gazebo 里瘫”对应 RViz 2 一节——RViz 2 只渲染消息、不计算物理；“Gazebo 里能站、行走偶尔抽一下”对应“ROS 2 集成与控制器”里仿真时序那一句（物理步长、接触求解、实时率与控制器更新节奏不匹配）；“rosbag 只录了关节状态、没录接触力只能猜”对应 Foxglove 一节的记录清单纪律。
