...
频率分层与 5.2 节的表一致：行为层秒级、规划层几十赫兹、控制层百赫兹以上；把这些不同节奏的模块塞进一个进程一个线程，是新人最常见的架构错误。
...
两者的分工：RViz 2 看空间关系，Foxglove 看时间序列。4.4–4.6 节与 5.2 节每一节的排查清单，最终都落在"这条曲线/这棵树长什么样"上。
