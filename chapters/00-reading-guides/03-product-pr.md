...
全书 34 节中有 22 节进了这条路线，合计约 1 小时 15 分钟。顺序与正文章节一致，可以顺着左侧边栏一路往下点；没进这条路线的小节不是不重要，只是对产品 / PR 岗位的单位收益更低，需要时再单独查。

## 第一步：建立整机概念（4 节，约 15 分钟）
...
| [1.4 从一句话到电机：整机总图](../01-system-overview/04-from-task-to-motor.mdx) | 整机总图：任务、行为、运动、控制、驱动、机械各层做什么、多快、跑在哪——“这句话是怎么变成电机转动的？”；两个闭环（在线运行／离线开发）紧随其后 |
