...
**读法约定**：每一节只读三样东西——开头的现场问题（读到最后那句被引用的话）、该节**第一个术语定义**（中英对照），以及**一条“一句话听懂”**（多数节紧跟在术语定义之后；少数节的第一条结论出现在术语之前，读到那条就算）。这三样大多在每节开头的约 1200 汉字以内（4.4 这类长节最晚到 1500 字左右，2.1 因为要先把机构术语铺开，最晚到 1700 字左右），平均每节约 3 分钟；遇到不懂的词，用页面顶部的搜索框。正文里出现“……**第一次阅读如果只是建立系统概念，到这里即可。**”这句话的地方，就是该节的**安全退出点**，看到它可以直接跳到下一节。
