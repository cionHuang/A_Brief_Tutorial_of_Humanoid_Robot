...
**读法约定**：每一节只读三样东西——开头的现场问题（读到最后那句被引用的话）、该节**第一个术语定义**（中英对照），以及**一条“一句话听懂”**（多数节紧跟在术语定义之后；少数节的第一条结论出现在术语之前，读到那条就算）。这三样都在每节开头的 {{stat:routes.product.maxReadHan}} 汉字以内（最长的是 {{stat:routes.product.maxReadSection}} 节，约 {{stat:routes.product.maxReadMinutes}} 分钟），平均每节 {{stat:routes.product.perSectionText}}；遇到不懂的词，用页面顶部的搜索框。正文里出现“……**第一次阅读如果只是建立系统概念，到这里即可。**”这句话的地方，就是该节的**安全退出点**，看到它可以直接跳到下一节。
...
| 小节 | 读完开头的术语与结论，你将听懂 |
| --- | --- |
| [1.1 我们到底在研究什么：具身智能与人形机器人](../01-system-overview/01-what-is-embodied-ai.mdx) | 弹幕里的“这是人在背后遥控的吧？”为什么问得专业：遥控、脚本、自主的差别在自主程度；具身智能要求的两件事：身体与闭环；“物理 AI”紧随其后 |
| [1.2 一台人形机器人里面有什么：以 G1 为例](../01-system-overview/02-humanoid-robot-anatomy.mdx) | “拆开之后到底由哪些东西组成？”：连杆、关节、自由度；执行器、传感器、末端执行器紧随其后 |
| [1.3 从任务到电机：一台现代人形机器人如何运行](../01-system-overview/03-humanoid-robot-system-architecture.mdx) | “这句话是怎么变成电机转动的？”：六层功能链、各层多快、算法跑在哪；两个闭环（在线运行／离线开发）紧随其后 |
...
| 小节 | 读完开头的术语与结论，你将听懂 |
| --- | --- |
| [2.1 机构、关节与自由度](../02-mechanics/01-mechanisms-and-dof.mdx) | 零位、限位——“同一组指令在仿真和实机上就可能变成两个动作”是怎么回事；死点紧随其后 |
...
- 涉及具体本体的参数（自由度、控制频率、关节数）以本书引用的小节为准，每个数字都有官方来源可核对；
