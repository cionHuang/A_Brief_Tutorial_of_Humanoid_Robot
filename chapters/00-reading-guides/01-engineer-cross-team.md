...
建议先花 30 分钟读 [1.3 从任务到电机：一台现代人形机器人如何运行](../01-system-overview/03-humanoid-robot-system-architecture.mdx)：它给出整张总图——六层功能链、各层频率、算法跑在哪、两个闭环，后面每一节都会回到这张图定位自己；然后对号入座：你这周要和谁开会、和谁联调，就按对应的表读。

本书正文里还标着**安全退出点**——提示框的第一句是“**第一次阅读如果只是建立系统概念，到这里即可。**”，读到它说明这一节的主线已经讲完，后面的推导与实现细节可以等你真要动手时再回头。
