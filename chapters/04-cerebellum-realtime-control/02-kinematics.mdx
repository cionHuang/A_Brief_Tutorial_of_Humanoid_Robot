...
本手册使用的 G1 无手、腰部可动模型，把 pelvis、腿、腰和双臂组织成一棵 body/joint 树。沿这条树把局部变换逐段乘起来，就能由关节角算出左脚、手腕或 IMU 安装点在 pelvis 中的位姿。下面用左臂做一次实时正运动学——连杆尺寸和关节限位都取自 G1 的 URDF：
...
| | 四连杆死点（[2.1 节](../02-mechanics/01-mechanisms-and-dof.mdx)） | 万向节锁（[4.1 节](./01-robot-mathematics-foundations.mdx)） | 奇异位形（本节） |
| --- | --- | --- | --- |
...
$\lambda$ 越大，越能抑制奇异附近的速度爆炸，但末端跟踪误差也会增大。阻尼无法“修复”奇异位形，它做的是在速度、误差和数值稳定性之间的可控折中；工程实现还应配合关节速度限幅、目标变化率限制和失败状态返回。本节对应的逆运动学演示里有一个 $\lambda$ 滑块：把目标推到工作空间边缘、再把 $\lambda$ 调小，就能看到关节速度暴涨或抖动——这正是奇异在数值上的表现。
