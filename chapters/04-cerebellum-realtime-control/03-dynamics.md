...
方程里反复出现的质量矩阵和各惯性项，用的就是 2.2 节讲过的质量、质心和转动惯量；区别是它们必须连同表达坐标系和惯性原点一起进入方程，而且会随姿态变化。
...
> **一句话听懂**：动力学方程就是“给机器人算总账”：惯性、科氏力、重力、摩擦这些阻力加在一起，必须由电机力矩、接触力和外力来平衡。移项顺序各家写法不同，关键是别把每一项的方向和坐标系搞错。
...
$\boldsymbol{\lambda}$ 不是可以随便填的常数，它的取值由接触约束和求解器共同确定；法向力非负、切向力受摩擦锥限制，这些都是在求解 $\boldsymbol{\lambda}$ 时施加的约束——摩擦锥的定义、以及“需要的切向力”与“上限 $\mu f_n$”的比较方法见 [2.4 节](../02-mechanics/04-foot-contact-and-stability.mdx)。仿真里的摩擦系数、接触刚度、阻尼和碰撞几何只代表指定场景的近似，同样不能当成真实地面或真实 G1 的规格。

> **一句话听懂**：脚踩地面这股力不能算在某个关节头上：它是整机与地面的相互作用，要经接触雅可比折算成关节力矩。它还守两条规矩——只能推不能拉、横向不超过摩擦锥，否则就打滑。

### 与前文足部和全身控制的联系

2.4 节回答的是“接触力够不够”：足底几何、摩擦锥和支撑多边形给出几何与准静态条件。本节补上动力学一侧：要实现同样的运动，还需要多大关节力矩、地面反力如何分配到各关节、以及接触约束是否可解。因此全身控制里常见的约束包括足底位姿、接触法向、摩擦锥、质心/动量变化和关节力矩上限——一个几何上可行的脚步，如果需要的地面反力超过摩擦或某个膝关节的持续力矩能力，仍然是动力学不可行的。
...
> **一句话听懂**：质量矩阵不只是公式里的符号：它能告诉你这个姿势下哪个方向好加速、哪个方向特别费劲，也能用来估峰值力矩、管动量和分配全身任务。但它只算机械侧的需求，电机铜损和减速器效率得另算。
