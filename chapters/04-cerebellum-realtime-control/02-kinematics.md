...
运动学（Kinematics）研究的是“几何构型如何决定位置、姿态、速度和力的映射”，暂时不讨论质量、惯量和力的产生原因。它是规划器、逆运动学、全身控制和碰撞检查之间的共同接口：输入通常是关节变量和模型几何，输出是连杆/末端位姿、速度映射、雅可比以及可行性指标。
...
本手册使用 `robot_descriptions/g1/g1_29dof.xml` 的 G1 无手、腰部可动模型作为主要示例。模型在 `pelvis` 下挂有浮动基座，并沿腿部、腰部和双臂的 body/joint 层级组织几何关系。左腿的一条简化链可以写成：
...
![G1 正运动学、逆运动学、雅可比与约束求解流程](assets/images/03-g1-kinematics-fk-ik-jacobian.png)

图：以 G1 `g1_29dof` 无手、腰部可动模型为例，展示从关节变量到左脚末端位姿的正运动学、从右腕目标反求关节变量的逆运动学、雅可比速度/力映射，以及关节限位、自碰撞、接触和奇异位形约束。图中浮动基座 6 DoF 不计入 29 个电机关节；四连杆部分为概念性数学对照，不代表 G1 的真实内部机构。
