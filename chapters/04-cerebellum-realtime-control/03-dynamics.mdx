...
这不表示 G1 只有 29 个总自由度，也不表示浮动基座有 7 个物理自由度；四元数的 4 个分量受单位长度约束，实际基座位姿仍是 6 DoF。动力学方程中，浮动基座对应的行通常没有直接电机力矩输入，是由地面接触、外力和全身关节动作共同决定的“欠驱动（Underactuated）”部分。[4][6]
...
[2.4 节](../02-mechanics/04-foot-contact-and-stability.mdx)回答的是“接触力够不够”：足底几何、摩擦锥和支撑多边形给出几何与准静态条件。本节补上动力学一侧：要实现同样的运动，还需要多大关节力矩、地面反力如何分配到各关节、以及接触约束是否可解。因此全身控制里常见的约束包括足底位姿、接触法向、摩擦锥、质心/动量变化和关节力矩上限——一个几何上可行的脚步，如果需要的地面反力超过摩擦或某个膝关节的持续力矩能力，仍然是动力学不可行的。
