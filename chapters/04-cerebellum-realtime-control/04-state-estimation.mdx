...
陀螺仪积分在秒级内可靠，但零偏会累积成越来越大的姿态误差；加速度计只在静止或低动态时提供可靠的重力方向，剧烈加速、碰撞和摆腿时会把运动加速度一起测进来。两者放在一起，roll/pitch 还能靠重力方向拉回来，yaw 却没有任何重力观测可参照——这就是 IMU 与编码器的根本分工：一个提供短时动态和局部姿态，一个提供关节的绝对几何。
