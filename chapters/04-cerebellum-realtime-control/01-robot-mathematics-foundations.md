...
![人形机器人中向量、旋转表示、齐次变换与坐标链的连接](assets/images/01-robot-mathematics-foundations.png)

图：以 G1 `g1_29dof` 无手模型为例，概览向量运算、World—pelvis—腿—足端坐标链、欧拉角/旋转矩阵/轴角/四元数，以及齐次变换和接口单位。图中欧拉角的万向节锁只作概念提示，具体奇异条件和它与四连杆死点的区别见后文。
