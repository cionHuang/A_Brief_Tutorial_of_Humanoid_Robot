...
对本手册使用的 G1 `g1_29dof` 无灵巧手（末端为模型自带的固定橡胶手）、腰部可动模型，`pelvis` 和 `torso` 是真实刚体坐标系，`imu_in_pelvis`、`imu_in_torso` 是传感器安装坐标系；它们的姿态仍由三维旋转描述。若调试工具把 IMU 的四元数转换成 ZYX roll-pitch-yaw，只应把欧拉角当作显示或配置接口，不能因为显示角跳变就修改 URDF/MJCF 中的关节轴线、骨盆位姿或传感器安装关系。[6][7]
