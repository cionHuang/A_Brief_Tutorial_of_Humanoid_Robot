...
| `g1_29dof_lock_waist.urdf` | URDF | 锁腰对比（4.6 节） | **27** | 仅 `waist_yaw` 可动；`waist_roll`/`waist_pitch` 为 fixed | 无 | 36 个 STL |
| `g1_29dof_with_hand.urdf` | URDF | 双臂操作、灵巧手与 VLA（5.4、5.6 节） | **43** = 29 + 14 | 三轴均可动 | 每手 7（拇指 3 + 食指 2 + 中指 2），双手 14 | 50 个 STL |
| `g1_29dof.xml` | MJCF | MuJoCo 仿真默认模型（6.2 节） | 29 个 `<motor>` 执行器 | 三轴均可动 | 无 | 36 个 STL |
...
- 三个 MJCF 都用 `<compiler angle="radian" meshdir="meshes"/>`，即角度单位为弧度、网格相对当前文件解析；移动文件时必须保持 `meshes/` 的相对位置（6.2 节）。
...
- **`scene.xml` 的 include 写的是 `g1_12dof.xml`，该文件不在本仓库**——使用前必须改成目标模型文件名（6.2 节的加载检查清单专门讲了这一点）。
- 关节索引、轴向与限位见[附录 B](./03-joint-index.md)；每条消息的字段见[附录 C](./04-messages.md)。
- 本仓库 `meshes/` 目录共 53 个 STL，上表“引用的网格”是各描述文件实际用到的子集；`images/` 目录提供 `g1_29dof.png` 与 `g1_29dof_with_hand.png` 两张示意图（供上游 README 引用）。
- 网站侧由 `site/scripts/sync-robot-assets.mjs` 把 `g1_29dof.urdf`、`g1_29dof_lock_waist.urdf` 及其引用的网格同步到 `site/public/models/g1/`，供 1.2 节的 G1 URDF 查看器在浏览器里加载。
